...
const getDataURL = ({host, version, siteId, endpoint}) => `${host}/s/-/dw/data/${version}/sites/${siteId}/${endpoint}`
const getShopURL = ({host, version, siteId, endpoint}) => `${host}/s/${siteId}/dw/shop/${version}/${endpoint}`
...
export const createOcapiFetch = (commerceAPIConfig) => async (
    ...
    let url
    if (args[0].data) {
        const newToken = await getToken(commerceAPIConfig.parameters, args[0].clientSecret)
        headers['Authorization'] = 'Bearer ' + newToken.accessToken;
        url = getDataURL({host, version: (version ? version : defaultVersion.data), siteId, endpoint})
    } else {
        url = getShopURL({host, version: (version ? version : defaultVersion.shop), siteId, endpoint})
    }
